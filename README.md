...
- **金海豚小游戏**：全屏识别经验星、爱心、宝石与贝币，可选择最高优先级，最多连续完成 3 局。
- **公主任务**：在右侧奖励列中查找可领取礼盒，点击命中位置并处理领取成功或失败结果后返回鱼缸。
- **绿野寻仙踪日常**：开 1 次贝壳后去商店买一条贝币鱼；独立任务进入绿野寻仙踪，打开任务并一键领取后返回。
...
4. 运行 `MaaHappyFish.exe`，选择安卓控制器与对应设备后执行任务。
