...
| **Latest release** | [`v0.5.1`](https://github.com/zhimaheiye/MaaHappyFish/releases/tag/v0.5.1) |
...
| 多鱼缸巡检 | 三缸收宝、30 秒无宝石切缸、通用海星页喂食、每小时扩展均已编码；逐步结果日志、60 秒等待心跳与专用开贝壳页面恢复已通过代码级验证 | 三缸收宝通过；开贝壳恢复复用既有 `贝壳页面_返回.png`，不使用通用活动页模板；本次未做模拟器测试，海星标签路由、魔力召唤三分支、宝石融合待测 | 保留“宝石日志精简与滑动拾取”待优化项；后续按需分阶段验证尚未实测的业务分支 |
...
| 钓鱼达人 | 六地点与日常聚合选项完成；早期咬钩检测已优化；结束时只点击钓场右上角，并确认主界面模板后再继续，不再退回地点地图后跨页盲点 | 退出问题完成代码级回归，本次未做模拟器测试；命中率最近一次 2/5，早期识别优化后未复测 | 下次五次机会记录 FPS、截图耗时和空杆数 |
...
**状态**: 三缸收宝与 30 秒无产物切缸已通过 MFA 实测。旧喂食流程会三次误入同一“乖海星”，且深海鱼食点击成功后因负向候选分支错误而超时；已改为只进入一次通用海星页面，再逐个验证顶部标签并按实际弹窗选择普通或深海鱼食。2026-09-11 已补齐三缸、海星、魔力召唤、宝石融合的逐步结果日志以及每 60 秒等待心跳；2026-09-12 复用既有 `贝壳页面_返回.png` 为巡检补上误入大章鱼开贝壳页面的专用恢复门禁，明确不使用通用活动页模板。上述改动均完成代码级验证，本次按用户要求未做模拟器测试；新喂食路由和每小时子任务仍待 Maa/MFA 实机验证。
...
- `assets/resource/pipeline/features/fishing.json` (38 节点)：钓鱼达人全链路导航、垂钓与退出验证流水线；
