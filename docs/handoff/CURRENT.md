...
| **Latest release** | [`v0.5.4`](https://github.com/zhimaheiye/MaaHappyFish/releases/tag/v0.5.4) |
...
| 多鱼缸巡检 | 三缸收宝、30 秒无宝石切缸、通用海星页喂食、每小时扩展均已编码；宝石逐枚日志已精简；开贝壳恢复改为页面本体门禁后再点击返回；SHAKE 每缸完成后强制进入图像识别复查，残留产物会继续收取且命中后重置 30 秒窗口；宝石融合首次返回未离页时会重新确认融合页并二次返回 | 三缸 IMAGE 收宝通过；2026-09-13 主力机日志确认 SHAKE 旧流程直接切缸与宝石融合首次返回只消弹窗，两项已完成代码修复和代码级验证，本次未做模拟器测试；海星标签路由与魔力召唤三分支待测 | 自然复测 SHAKE 后图像复查及宝石融合弹窗二次返回；后续按需分阶段验证其他未实测业务分支 |
...
| 钓鱼达人 | 六地点与日常聚合选项完成；早期咬钩检测已优化；鱼饵耗尽覆盖各运行阶段；退出改为识别 `钓鱼达人_退出.png` 并点击实际命中位置，确认主鱼缸后继续 | 新退出模板仅完成代码级验证，未做模拟器测试；命中率最近一次 2/5，早期识别优化后未复测 | 下次五次机会同时核对退出模板、FPS、截图耗时和空杆数 |
...
| 开贝壳、浪漫满屋 | 开贝壳新增主鱼缸入口识别和次数完成后的 OCR 返回、主鱼缸确认；浪漫满屋已完成 | 原有两功能闭环已有实机记录；开贝壳新增入口与返回仅完成代码级验证，本次未做模拟器测试 | 自然复测开贝壳从鱼缸进入及完成后返回；浪漫满屋仅做回归 |
...
### 开贝壳活动 (`OpenShellTask`)
- [x] 多轮开贝壳闭环自动化；支持从活动主页直接启动，或在主鱼缸识别 `开贝壳_入口.png` 后进入
...
- [x] 五张用户模板、ROI、界面任务入口与四类全局弹窗恢复均由专项静态测试锁定（代码级通过，本次未做模拟器测试）
...
- [ ] 开贝壳新增路径实测：从主鱼缸命中入口进入活动页；达到设定次数后识别返回并确认主鱼缸
...
- [ ] 巡检宝石融合：空闲点击合成、结果弹窗放入仓库后再次合成、正在融合直接返回三条分支；首次返回只关闭临时弹窗时的融合页复验与二次返回已完成代码修复，待自然实测。
...
**状态**: 三缸 IMAGE 收宝与 30 秒无产物切缸已通过 MFA 实测。旧喂食流程会三次误入同一“乖海星”，且深海鱼食点击成功后因负向候选分支错误而超时；已改为只进入一次通用海星页面，再逐个验证顶部标签并按实际弹窗选择普通或深海鱼食。2026-09-12 报告确认 `贝壳页面_返回.png` 会以 0.999998 误命中鱼缸管理页同款返回按钮；现已接入用户提供的 `开贝壳_识别.png` 页面本体门禁，只有确认大章鱼页面后才点击返回。逐枚宝石日志也已按用户要求移除。2026-09-13 主力机日志进一步确认 SHAKE 旧路径最终扫底后直接切缸会漏掉残留产物，现已强制转入 30 秒图像复查；宝石融合第一次返回若只关闭临时弹窗，会重新确认融合页面后第二次返回。上述改动完成代码级验证，本次未做模拟器测试；新喂食路由和其他每小时子任务分支仍待 Maa/MFA 实机验证。
...
**状态**: ✅ 已完成模块化拆分及后续功能扩展，当前共 435 节点。

**拆分架构**：
- `assets/resource/pipeline/common/common.json` (15 节点)：应用启动、公告关闭、进游戏、主界面确认，以及活动页面退出、每日签到、特惠礼包、快报全局处理；
...
- `assets/resource/pipeline/features/open_shell.json` (15 节点)：主鱼缸活动入口、大章鱼页面门禁、开贝壳循环、完成返回与主鱼缸确认；
...
- `assets/resource/pipeline/features/patrol.json` (74 节点)：多鱼缸收宝、SHAKE 后图像复查、每次气泡后的安全滑动、开贝壳页面恢复门禁、管理页三缸海星喂食与间隔等待循环；
- `assets/resource/pipeline/features/patrol_extras.json` (24 节点)：巡检内每小时魔力召唤三状态分支与宝石融合可选子任务，含“正在执行”显式结果节点及临时弹窗后二次返回；
...
- `assets/resource/pipeline/collect_fish.json` (40 节点)：收鱼主干、每次气泡后的底部安全滑动及误入开贝壳页面的两阶段恢复；
- 原 `my_task.json` 备份为 `my_task.json.bak`（防 Maa 重复 key 冲突，安全保留回滚途径）。

**验证结果**：
- 当前 Pipeline 节点总数：435；
