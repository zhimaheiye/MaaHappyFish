...
| **Latest release** | [`v0.5.2`](https://github.com/zhimaheiye/MaaHappyFish/releases/tag/v0.5.2) |
...
| --- | --- | --- | --- |
| 收鱼产物、单次海星喂食、鱼食预算 | 已完成；所有产物气泡点击后均衔接底部安全滑动；误入开贝壳恢复已改为页面本体门禁后再点击返回 | 原核心闭环已验证；新增安全滑动及开贝壳页面门禁待用户自然回归 | 继续观察长时间稳定性、双倍奖励效果和误入页面恢复 |
| 购买廉价鱼食 | 已完成，含停止检查与步骤恢复 | 用户已确认成功 | 无；后续只在廉价鱼食视觉门禁下充分回归 |
| 每日免费礼包 | 可领取/已售罄两条并列分支完成 | 两条分支均通过 | 完备，回归即可 |
| 全局活动页/签到/特惠礼包 | 活动页退出、签到和特惠礼包均已接入全局检查点；签到保持先领取后关闭 | 活动页退出已按用户截图完成代码级接入，本次未做模拟器测试。签到领取能点击，修正后的关闭与业务恢复待自然弹窗复测；特惠礼包待串联复测 | 后续自然出现时观察返回鱼缸后的原任务恢复情况 |
| 多鱼缸巡检 | 三缸收宝、30 秒无宝石切缸、通用海星页喂食、每小时扩展均已编码；宝石逐枚日志已精简；开贝壳恢复改为页面本体门禁后再点击返回 | 三缸收宝通过；报告已确认旧返回按钮门禁会误退鱼缸管理页，新 `开贝壳_识别.png` 门禁仅完成代码级验证；海星标签路由、魔力召唤三分支、宝石融合待测 | 滑动拾取等待实机取证；后续按需分阶段验证尚未实测的业务分支 |
| 好友摸宝 | 已完成；新增主鱼缸好友入口，周末遇到海牛先生时由跳过改为执行共享喂食流程 | 原 200 位好友 44 分钟 E2E 通过；新增入口和海牛集成仅代码级通过 | 周末验证海牛接管后能够恢复下一位好友 |
| 海牛先生 | 周末独立任务完成；至少喂食 30 次并持续检查刷新体力，独立双返回和好友摸宝恢复分支均已编码 | 本次未做模拟器测试 | 周末分别验证独立任务与好友摸宝接管流程 |
| 宝石礼盒兑换 | 独立任务完成；首轮实机发现页面标题门禁漏写“鱼”导致动作未启动，已修正；七配方按等级识别，OK 仅作状态门禁并点击配方文字与 OK 中点 | 修复后仅完成代码级验证，未做模拟器复测 | 用户复测页面门禁、卡片中点点击、七配方顺序和 10/10 更新 |
...
- [x] 未完成卡片严格执行：识别同卡片 OK → 点击配方文字与 OK 中点（基准 `(360,215)`）→ 确认兑换弹窗 → 识别加号并点击 9 次 → OCR 点击兑换 → OCR 点击确定 → 复核 10/10
