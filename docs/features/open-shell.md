...
- 已在一轮中途：按 `随机奖品 → 太好了 → 继续开贝壳 → 打开贝壳` 的最深阶段顺序恢复；已经看到“打开贝壳”时只继续本轮，绝不重复点击“立即开始”。外部启动的半轮在回到“立即开始”页后由既有循环计数只记一次。
- 已在大章鱼活动主页：在 `[37,68,226,142]` 用 `开贝壳_识别.png` 确认页面，进入“立即开始”流程。
...
- 2026-09-24 增加 `随机奖品 → 太好了 → 继续开贝壳 → 打开贝壳` 四个半轮启动候选，优先于待开始页；专项拓扑、正则/资源加载和引用门禁通过，未操作模拟器。
