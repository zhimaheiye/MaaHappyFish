...
- `贝壳页面_返回.png` 是项目已经存在的大章鱼开贝壳页面专用返回模板，识别 ROI 为 `[0,0,250,150]`；`CollectFishTask` 的 `HandleShellPage` 已使用该资产处理误入页面。
- 其他任务需要处理误入大章鱼页面时应复用这张专用模板和真实命中点击，不得改用 `活动页面_退出.png` 通用活动页模板。多鱼缸巡检通过 `[JumpBack]PatrolShellPagePopup` 复用该契约。
