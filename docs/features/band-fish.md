...
**最后更新**: 2026-09-22
**状态**: 导航与邀请流程已实现；用户已确认“最新乐章”从选曲、演奏到结算的完整流程通过 MFA 实测。现已按列表顺序录入全部 19 首指定乐章，并增加从顶部逐屏下滑查找及唯一 OCR 关键词兼容；所有指定乐章均尚未 MFA 实测。v0.5.0 台式机日志暴露的槽位 2 首屏 OCR 缺首字问题已完成代码修复与专项验证，本次未做模拟器测试。2026-09-22 首次独立任务全流程实测：4 位人机好友邀请一次全部成功，“最新乐章”演出结算闭环成功；但发现"邀请后人机好友约 3.5 分钟才就绪"，PENDING 重进循环（约 80 秒窗口）等不到就绪即判失败，人工重跑即成功，详见 §2026-09-22 复盘。
...
2. **PENDING 循环缺少"等待好友接受"的手段**：每轮重进后 `BandFishInviteLoopAction` 只扫描绿色邀请按钮（结果恒为空槽 `[]`），既不检测"开始演出"按钮，也无退避等待；重试配额（约 6 轮 / 80 秒）耗尽后任务直接判失败。失败时 MFA 日志只有 Monitor 一行"任务失败"，无 WRN/ERR，`debug/on_error` 亦无截图，排障只能靠 Stdout 流还原。
3. 就绪判定本身没有问题：第 2 次运行同样的入口、同样的 OCR，7 秒即命中"开始演出"。

**改进建议**（待排期）：

- [ ] PENDING 状态下若扫描到"空槽为空但未就绪"，改为**长退避等待**（如 30s/60s/120s 递增，总窗口 ≥ 5 分钟）再重进，而不是 13 秒一轮的固定快循环；
- [ ] 或在重进循环中并入 `BandFishCheckReady` 的"开始演出" OCR 检测，就绪即直接进入演出分支，避免"邀请动作短路 Ready 检测"；
- [ ] 邀请全部发出后先回主鱼缸**等待固定缓冲**（如 60~90 秒）再开始重进循环；
- [ ] 任务失败时在 `debug/on_error` 落一张现场截图，便于区分"好友未就绪"与其它失败形态。
