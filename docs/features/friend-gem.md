...
### 7. 收宝石双模式支持（IMAGE / SHAKE）
2026-09-12 支持根据任务选项“收宝石方式”选择模式：
- **IMAGE（图像识别，默认）**：保持原有开阔水域气泡识别与逐个点击逻辑；
- **SHAKE（MuMu摇晃）**：在确认好友水族箱且未处于耗尽状态时，执行一次摇晃收宝循环（5 次 shake + 6 次扫底），完成后由 `FriendGemNextFriend` 直接切往下一位好友，极大提升巡访效率；连续 3 次失败安全熔断。
