...
class PipelineRateLimitReco(CustomRecognition):
...
res = Resource()
reco = PipelineRateLimitReco(target_samples=50)
res.register_custom_recognition("PipelineRateLimitReco", reco)

# 测试在 Pipeline 中配置 rate_limit: 10
pipeline_override = {
    "RateLimitTestEntry": {
        "recognition": "Custom",
        "custom_recognition": "PipelineRateLimitReco",
        "rate_limit": 10,
        "action": "DoNothing",
        "next": [
            "RateLimitTestEntry"
...
print("Starting Pipeline test with MuMuPlayerExtras and rate_limit=10 (50 samples)...")
job = tasker.post_task("RateLimitTestEntry", pipeline_override)
job.wait()

dts = np.array(reco.timestamps)
print(f"Pipeline with MuMuPlayerExtras + rate_limit=10 (50 samples):")
